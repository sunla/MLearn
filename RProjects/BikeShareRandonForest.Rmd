...
temppartial <- partialPlot(rf, features, names(features)[4], plot=FALSE)
temppartial$x

tempfeatures <- cbind(tempfeatures,rf$)

ggplot(tempfeatures, aes(x=temp, y= rf$y)) + geom_smooth()
tempfeatures <- cbind(internal_train,rf$y)

levels(as.factor(train$temp))
rf
rf$
